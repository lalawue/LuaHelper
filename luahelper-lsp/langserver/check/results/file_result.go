...
func (f *FileResult) CheckReferFile(referInfo *common.ReferInfo, allFilesMap map[string]string, fileIndexInfo *common.FileIndexInfo) {
	...
	if suffixFlag {
		...
		if !common.GConfig.ReferMatchPathFlag {
			...
			bestFilePath := common.GConfig.GetDirManager().GetBestMatchReferFile(curFile, strFile, allFilesMap, fileIndexInfo)
			if bestFilePath != "" {
				// lua文件存在，正常
	...
	strBestFileTmp := dirManager.GetBestMatchReferFile(curFile, strNewFile, allFilesMap, fileIndexInfo)
	if strBestFileTmp != "" {
		// 匹配到了，判断对应的文件，是否存在
		// lua文件存在，正常
	...
	initFile := strNewFile + "/init.lua"
	// 如果配置为全路径匹配，尝试模糊匹配路径
	strBestFileTmp = dirManager.GetBestMatchReferFile(curFile, initFile, allFilesMap, fileIndexInfo)
	if strBestFileTmp != "" {
		referInfo.ReferValidStr = strBestFileTmp
		return
	}
