...
func (p *luaParser) parsePrefixExp() ast.Exp {
	l := p.l
	var exp ast.Exp
...
func (p *luaParser) parseParensExp() ast.Exp {
	l := p.l
...
func (p *luaParser) finishPrefixExp(exp ast.Exp, beginLoc *lexer.Location) ast.Exp {
	l := p.l
	for {
...
// functioncall ::=  prefixexp args | prefixexp ‘:’ Name args
func (p *luaParser) finishFuncCallExp(prefixExp ast.Exp) *ast.FuncCallExp {
	l := p.l
	beginLoc := l.GetNowTokenLoc()
...
func (p *luaParser) parseNameExp() *ast.StringExp {
	l := p.l
...
func (p *luaParser) parseArgs() (args []ast.Exp) {
	l := p.l
	switch l.LookAheadKind() {
