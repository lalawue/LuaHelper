...
func createLuaParser(chunk []byte, chunkName string) *luaParser {
	parser := &luaParser{}
...
// BeginAnalyze 开始分析
func (p *luaParser) BeginAnalyze() (block *ast.Block, commentMap map[int]*lexer.CommentInfo, errList []lexer.ParseError) {
	defer func() {
...
func (p *luaParser) BeginAnalyzeExp() (exp ast.Exp) {
	defer func() {
...
func (p *luaParser) GetErrList() (errList []lexer.ParseError) {
	return p.parseErrs
}

// insert now token info
func (p *luaParser) insertParserErr(loc lexer.Location, f string, a ...interface{}) {
	err := fmt.Sprintf(f, a...)
...
func (p *luaParser) insertErr(oneErr lexer.ParseError) {
