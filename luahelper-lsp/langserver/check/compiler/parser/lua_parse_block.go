...
// block ::= {stat} [retstat]
func (p *luaParser) parseBlock() *ast.Block {
	return &ast.Block{
		Stats:   p.parseStats(),
...
func (p *luaParser) parseStats() []ast.Stat {
...
// explist ::= exp {‘,’ exp}
func (p *luaParser) parseRetExps() []ast.Exp {
	l := p.l
...
func (p *luaParser) isReturnOrBlockEnd(tokenKind lexer.TkKind) bool {
	switch tokenKind {
